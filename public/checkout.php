...
$cfg_stmt = $pdo->query("SELECT chave, valor FROM configuracoes
    WHERE chave IN ('loja_whatsapp','entrega_taxa','entrega_gratis',
                    ...
                    'retirada_ativa','retirada_tempo','pag_dinheiro',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $troco    = (float)str_replace([',','R$',' '],['.','',$_ = ''],$_POST['troco'] ?? 0);
    $obs      = trim($_POST['obs']      ?? '');
    ...
    // Itens do carrinho (JSON enviado pelo JS)
...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .nav-cart  { position:relative; width:40px; height:40px; border-radius:50%; background:var(--rosa-claro); border:none; cursor:pointer; display:flex; align-items:center; justify-content:center; }
        .nav-cart svg { width:17px; height:17px; stroke:var(--rosa); fill:none; stroke-width:2; }
        ...
        .campo-full   { /* ocupa linha inteira */ }
        ...
        .end-salvo {
            ...
            border:1.5px solid var(--rosa-borda); border-radius:var(--r);
        ...
        .end-novo-btn { font-size:.8rem; color:var(--rosa); font-weight:600; cursor:pointer; background:none; border:none; padding:4px 0; text-decoration:underline; }

        /* TIPO ENTREGA */
        .entrega-opts { display:flex; gap:10px; margin-bottom:18px; }
        .entrega-opt  { flex:1; border:1.5px solid var(--rosa-borda); border-radius:var(--r); padding:14px; cursor:pointer; transition:all .2s; display:flex; align-items:center; gap:10px; }
        .entrega-opt:has(input:checked) { border-color:var(--rosa); background:var(--rosa-claro); }
        .entrega-opt input[type="radio"] { accent-color:var(--rosa); width:16px; height:16px; flex-shrink:0; }
        .entrega-opt svg { width:20px; height:20px; stroke:var(--cinza); fill:none; stroke-width:1.8; flex-shrink:0; }
        .entrega-opt:has(input:checked) svg { stroke:var(--rosa); }
        .entrega-opt-titulo { font-size:.88rem; font-weight:600; }
        .entrega-opt-sub    { font-size:.75rem; color:var(--cinza); margin-top:2px; }
        ...
        @media (max-width:560px) {
            ...
            .entrega-opts { flex-direction:column; }
        }
    </style>
</head>
<body>
...
<div class="container">
    ...
    <div class="sucesso-box">
        ...
        <p>
            ...
            Agora clique abaixo para enviar pelo WhatsApp e confirmar com a gente.
        </p>
        ...
        <br>
        <a href="index.php" class="btn-rosa" style="margin-top:8px">Voltar ao início</a>
    </div>

    <script>
    // Limpa carrinho após pedido confirmado
    ...
    <div class="page-titulo">
        ...
        <p>Preencha seus dados. O pedido será enviado pelo WhatsApp.</p>
    </div>
    ...
    <form method="POST" id="formCheckout" style="display:none">
        <!-- Campo oculto com JSON do carrinho — preenchido pelo JS -->
        ...
        <div class="checkout-grid">
            ...
            <div>
                    ...
                    <div class="campo-grid">
                        ...
                        <div class="campo">
                            ...
                            <input type="tel" id="tel" name="tel"
                                placeholder="(81) 99999-9999" autocomplete="tel" required>
                        </div>
                    </div>
                    <?php if (!$logado): ?>
                    <p style="font-size:.78rem;color:var(--cinza);margin-top:10px;">
                        Tem uma conta?
                        <a href="login.php?volta=checkout.php" style="color:var(--rosa);font-weight:600;">Entrar</a>
                ...
                <div class="sec">
                    ...
                    <div class="entrega-opts">
                        ...
                        <label class="entrega-opt">
                            <input type="radio" name="tipo_entrega" value="entrega" checked onchange="toggleEntrega()">
                            <svg viewBox="0 0 24 24"><rect x="1" y="3" width="15" height="13"/><polygon points="16 8 20 8 23 11 23 16 16 16 16 8"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/></svg>
                            <div>
                                <div class="entrega-opt-titulo">Entrega</div>
                                <div class="entrega-opt-sub">~<?= $entrega_tempo ?> min</div>
                            </div>
                        </label>
                        <?php endif; ?>
                        <?php if ($retirada_ativa): ?>
                        <label class="entrega-opt">
                            <input type="radio" name="tipo_entrega" value="retirada" onchange="toggleEntrega()">
                            <svg viewBox="0 0 24 24"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
                            <div>
                                <div class="entrega-opt-titulo">Retirada no local</div>
                                <div class="entrega-opt-sub">~<?= $retirada_tempo ?> min • Grátis</div>
                            </div>
                        </label>
                        <?php endif; ?>
                    </div>
                    ...
                    <div id="camposEndereco">
                        ...
                        <p style="font-size:.8rem;font-weight:600;color:var(--escuro);margin-bottom:10px;">Seus endereços:</p>
                        <div class="ends-salvos" id="endsSalvos">
                            ...
                            <label class="end-salvo">
                                <input type="radio" name="end_salvo" value="novo" onchange="mostrarCamposEnd(true)">
                                <div class="end-salvo-info">
                        ...
                        <div id="formEndereco" <?= !empty($enderecos_salvos) ? 'style="display:none"' : '' ?>>
                            ...
                            <label class="check-row">
                                <input type="checkbox" name="salvar_endereco" value="1">
                                Salvar endereço para próximos pedidos
                            </label>
                            <div id="apelidoEndWrap" style="display:none;margin-top:10px">
                                <div class="campo">
                                    <label for="apelido_end">Apelido do endereço</label>
                                    <input type="text" id="apelido_end" name="apelido_end" placeholder="Ex: Casa, Trabalho...">
                                </div>
                            </div>
                            <?php endif; ?>
                        </div>
                    ...
                    <div class="pagto-opts">
                        ...
                        <label class="pagto-opt">
                            ...
                            <div>
                                ...
                                <div class="pagto-opt-sub">Pagamento na hora, mais rápido</div>
                            </div>
                        </label>
                        <?php endif; ?>
                        <?php if ($pag_cartao): ?>
                        <label class="pagto-opt">
                            ...
                            <div>
                                ...
                                <div class="pagto-opt-sub">Débito ou crédito na entrega</div>
                            </div>
                        </label>
                        <?php endif; ?>
                        <?php if ($pag_dinheiro): ?>
                ...
                <div class="sec">
                    ...
                    <div class="campo">
                        <textarea name="obs" id="obs" placeholder="Alguma observação geral sobre o pedido?" rows="3"></textarea>
                    </div>
                </div>
            ...
            <div>
                <div class="resumo">
                    ...
                    <div class="resumo-linha" id="linhaDesconto" style="display:none">
                        <span>Desconto</span>
                        <span id="resumoDesconto" style="color:#16a34a"></span>
                    </div>
                    <div class="resumo-linha" id="linhaFrete">
                        <span>Entrega</span>
                        <span id="resumoFrete">R$ <?= number_format($taxa_entrega, 2, ',', '.') ?></span>
                    </div>
...
var DESCONTO_TIPO = ''; // 'fixo' ou 'percentual'
var DESCONTO_VALOR= 0;

// ═══════════════════════════════════════════════════════
// CARRINHO — lê do localStorage
// ═══════════════════════════════════════════════════════
...
function atualizarBadge(){
    var total = cart.reduce(function(s,i){ return s + i.qtd; }, 0);
...
function renderResumo(){
    ...
    var tipo     = document.querySelector('input[name="tipo_entrega"]:checked');
    var tipoVal  = tipo ? tipo.value : 'entrega';
    ...
    var desconto = 0;
    if (DESCONTO_ATIVO > 0){
        if (DESCONTO_TIPO === 'percentual') desconto = subtotal * (DESCONTO_VALOR / 100);
        else desconto = Math.min(DESCONTO_VALOR, subtotal);
    }
    ...
    cart.forEach(function(it){
        ...
        html += '<div class="resumo-item">'
              + '<div><div class="resumo-item-nome">' + it.nome + (it.qtd > 1 ? ' x'+it.qtd : '') + '</div>'
    ...
    if (tipoVal === 'entrega' && GRATIS_ACIMA > 0){
...
function toggleEntrega(){
    var val = document.querySelector('input[name="tipo_entrega"]:checked').value;
    var campos = document.getElementById('camposEndereco');
    if (campos) campos.style.display = val === 'entrega' ? 'block' : 'none';
    renderResumo();
}
...
// Mostrar formulário de novo endereço ao clicar em "Novo"
function mostrarCamposEnd(forcar){
...
// Se tem endereços salvos, preenche o principal automaticamente
<?php if (!empty($enderecos_salvos)): ?>
(function(){
...
// Salvar endereço checkbox
var cbSalvar = document.querySelector('input[name="salvar_endereco"]');
if (cbSalvar){
...
function verificarCupom(){
    ...
    if (!codigo){ msg.textContent = 'Digite um código de cupom.'; msg.className = 'cupom-msg err'; return; }
...
function prepararSubmit(e){
    ...
    // Serializa carrinho para o campo oculto
