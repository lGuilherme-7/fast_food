        ...
        if ($qtd <= 0) {
            ...
            if ($prod) {
                if ($acao === 'saida' && $qtd > (int)$prod['estoque']) {
                    ...
                    $pdo->prepare("
                        INSERT INTO estoque_historico (produto_id, tipo, quantidade, motivo, admin_id)
                        VALUES (?, ?, ?, ?, ?)
                    ...
                    $sinal    = $acao === 'entrada' ? '+' : '-';
                    $mensagem = $sinal . $qtd . ' unidades em "' . $prod['nome'] . '". Novo estoque: ' . $novo . ' un.';
                }
            }
        }
    }
...
$sql    = "
    SELECT p.id, p.nome, p.estoque, p.estoque_min AS minimo,
           p.custo, p.ativo, p.imagem_url AS img, c.nome AS cat
    FROM produtos p
...
$stmt->execute($params);
$produtos_todos = $stmt->fetchAll(); // array normal, sem problemas com iterator_to_array
...
$stats = $pdo->query("
    SELECT
        ...
        COALESCE(SUM(estoque * custo), 0)                AS valor_total
    FROM produtos
")->fetch();
...
// ── CONTAGENS POR STATUS (todos os produtos, sem filtro) ──────
...
// ── ALERTAS (sem estoque ou abaixo do mínimo) ─────────────────
$criticos = array_values(array_filter($produtos_todos, fn($p) => statusEstoque($p) !== 'ok'));

// ── HISTÓRICO (últimas 10 movimentações reais) ────────────────
$historico = $pdo->query("
    SELECT
        h.tipo,
        h.quantidade AS qtd,
...
if ($modal_id > 0) {
    $stmt = $pdo->prepare("
        SELECT p.id, p.nome, p.estoque, p.estoque_min AS minimo, p.custo, p.imagem_url AS img
...
<head>
    ...
    <style>
        ...
        .filtros-linha .sel-cat { padding:9px 12px; border-radius:var(--r); border:1px solid var(--borda); background:var(--branco); font-family:var(--f-corpo); font-size:.85rem; color:var(--escuro); cursor:pointer; }
        .filtros-linha .busca-wrap { flex:1; min-width:160px; }
        ...
        /* Barra de estoque */
        .estq-cell { display:flex; align-items:center; gap:8px; min-width:100px; }
    ...
    <div class="main">
        ...
        <div class="conteudo">
            ...
            <div class="alerta-warn">
                ...
                <?= (int)$stats['sem_estoque'] ?> produto<?= (int)$stats['sem_estoque'] !== 1 ? 's' : '' ?> sem estoque<?= (int)$stats['estoque_baixo'] > 0 ? ' e ' . (int)$stats['estoque_baixo'] . ' com estoque baixo' : '' ?>. Verifique os alertas.
            </div>
            <?php endif; ?>
            ...
            <div class="painel-grid">
                ...
                <!-- PAINEL ESQUERDO: tabela de produtos -->
                <div>
                    ...
                    <div class="toolbar">
                        <div class="toolbar-esq">
                            <?php
                            $tabs = [
                                'todos' => 'Todos ('         . count($produtos_todos) . ')',
                                'zero'  => 'Sem estoque ('   . $qtd_zero  . ')',
                                'baixo' => 'Baixo ('         . $qtd_baixo . ')',
                                'ok'    => 'Normal ('        . $qtd_ok    . ')',
                            ];
                            ...
                            <table class="tabela">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="col-financeiro">Valor total</th>
                                        <th>Status</th>
                                        <th>Ações</th>
                                    </tr>
                                </thead>
                ...
                <!-- PAINEL DIREITO: alertas + histórico -->
                <div class="painel-dir">
                    ...
                    <div class="side-card">
                        ...
                        <?php else: ?>
                        <?php foreach ($criticos as $p): $st = statusEstoque($p); ?>
                        <div class="alerta-item">
                            ...
                            <div class="alerta-info">
                                ...
                                <div class="alerta-sub">
                                    <?= $st === 'zero'
                                        ? 'Sem estoque'
                                        : (int)$p['estoque'] . ' un. (mín: ' . (int)$p['minimo'] . ')' ?>
                                </div>
                            </div>
                    ...
                    <div class="side-card">
                        ...
                        <div class="side-vazio">Nenhuma movimentação registrada ainda.</div>
                        <?php else: ?>
                            ...
                            <div class="hist-info">
                                ...
                                <div class="hist-meta">
                                    ...
                                    <span class="hist-qtd <?= $h['tipo'] ?>">
                                        <?= $h['tipo'] === 'entrada' ? '+' : ($h['tipo'] === 'saida' ? '-' : '±') ?><?= (int)$h['qtd'] ?>
                                    </span>
                                    <?php endif; ?>
                                </div>
                            </div>
        ...
        <div class="modal-body">
            ...
            <div class="modal-prod">
                ...
                <div>
                    ...
                    <div class="modal-prod-estq">
                        Estoque atual: <strong><?= (int)$modal_prod['estoque'] ?></strong> un.
                        — mín. <?= (int)$modal_prod['minimo'] ?>
                    </div>
                </div>
            </div>
            ...
            <form method="POST"
                  action="estoque.php?status=<?= $filtro_status ?>&cat=<?= urlencode($filtro_cat) ?><?= $filtro_busca ? '&busca='.urlencode($filtro_busca) : '' ?>"
                  id="formMov">
                ...
                <!-- Painel quantidade (entrada / saída) -->
                <div id="painelQtd" <?= $modal_tipo === 'ajuste' ? 'style="display:none"' : '' ?>>
                    <div class="campo">
                        <label for="f-qtd">Quantidade</label>
                        <input type="number" id="f-qtd" name="qtd" min="1" placeholder="0">
                    </div>
                </div>
                ...
                <div class="campo">
                    ...
                    <input type="text" id="f-motivo" name="motivo"
                        placeholder="Ex: Reposição, produção diária, desperdício...">
                </div>
...
function setTipo(tipo) {
    ...
    var pQtd   = document.getElementById('painelQtd');
    var pAjust = document.getElementById('painelAjuste');
    ...
    if (tipo === 'ajuste') {
        pQtd.style.display = 'none'; pAjust.style.display = '';
        btn.style.background = 'var(--rosa)';
        btn.innerHTML = svg + 'Salvar ajuste';
    } else if (tipo === 'entrada') {
        pQtd.style.display = ''; pAjust.style.display = 'none';
        btn.style.background = '#22c55e';
        btn.innerHTML = svg + 'Registrar entrada';
    } else {
        pQtd.style.display = ''; pAjust.style.display = 'none';
        btn.style.background = '#f59e0b';
        btn.innerHTML = svg + 'Registrar saída';
    }
}
