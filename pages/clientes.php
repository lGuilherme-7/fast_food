...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['acao'])) {
    ...
    if ($acao === 'bloquear' && $cid > 0) {
        $pdo->prepare("UPDATE clientes SET ativo = NOT ativo WHERE id = ?")->execute([$cid]);
        $novo     = (int)$pdo->query("SELECT ativo FROM clientes WHERE id = $cid")->fetchColumn();
        $mensagem = 'Cliente ' . ($novo ? 'desbloqueado' : 'bloqueado') . ' com sucesso.';
    }

    if ($acao === 'excluir' && $cid > 0) {
        // Verifica se tem pedidos — apenas dissocia, não excluímos pedidos
        $stmt = $pdo->prepare("SELECT nome FROM clientes WHERE id = ?");
        $stmt->execute([$cid]);
        $nome_del = $stmt->fetchColumn();

        // Desvincula pedidos (nullable FK)
        $pdo->prepare("UPDATE pedidos SET cliente_id = NULL WHERE cliente_id = ?")->execute([$cid]);
        $pdo->prepare("DELETE FROM clientes WHERE id = ?")->execute([$cid]);
        $mensagem = 'Cliente "' . $nome_del . '" removido. Pedidos mantidos sem vínculo.';
    }
}
...
// ── QUERY PRINCIPAL ──────────────────────────────────────────
// Busca clientes com contagens reais de pedidos e gasto total
$sql    = "
    SELECT
        c.id,
        c.nome,
        ...
        c.telefone    AS tel,
        c.ativo,
        DATE_FORMAT(c.criado_em, '%d/%m/%Y')   AS cadastro,
        COUNT(p.id)                             AS pedidos,
        COALESCE(SUM(CASE WHEN p.status != 'cancelado' THEN p.total END), 0) AS gasto,
        DATE_FORMAT(MAX(p.criado_em), '%d/%m/%Y') AS ultimo
    FROM clientes c
...
if ($filtro_status === 'ativo')   { $sql .= " AND c.ativo = 1"; }
if ($filtro_status === 'inativo') { $sql .= " AND c.ativo = 0"; }
...
// Ordenação no SQL — mais eficiente do que ordenar em PHP
switch ($ordenar) {
    case 'gasto':   $sql .= " ORDER BY gasto DESC";            break;
    case 'nome':    $sql .= " ORDER BY c.nome ASC";            break;
    case 'recente': $sql .= " ORDER BY MAX(p.criado_em) DESC"; break;
    default:        $sql .= " ORDER BY pedidos DESC";          break;
}
...
// Gasto e ticket médio globais
$stats_gasto = $pdo->query("
    SELECT
        COALESCE(SUM(CASE WHEN p.status != 'cancelado' THEN p.total END), 0) AS total_gasto,
        COUNT(DISTINCT p.id) AS total_pedidos
    FROM pedidos p
    WHERE p.cliente_id IS NOT NULL
")->fetch();

$total_gasto  = (float)$stats_gasto['total_gasto'];
$ticket_medio = (int)$stats_gasto['total_pedidos'] > 0
    ? $total_gasto / (int)$stats_gasto['total_pedidos']
    : 0;
...
if ($ver_id > 0) {
    $stmt = $pdo->prepare("
        SELECT
            c.id, c.nome, c.email, c.telefone AS tel, c.ativo,
            DATE_FORMAT(c.criado_em, '%d/%m/%Y')      AS cadastro,
            COUNT(p.id)                                AS pedidos,
            COALESCE(SUM(CASE WHEN p.status != 'cancelado' THEN p.total END), 0) AS gasto,
            DATE_FORMAT(MAX(p.criado_em), '%d/%m/%Y')  AS ultimo
    ...
    // Últimos 5 pedidos do cliente para o modal
    if ($detalhe) {
        $stmt = $pdo->prepare("
            SELECT id, total, status, DATE_FORMAT(criado_em, '%d/%m/%Y %H:%i') AS data
            FROM pedidos WHERE cliente_id = ?
            ORDER BY criado_em DESC LIMIT 5
        ");
        $stmt->execute([$ver_id]);
        $detalhe['ultimos_pedidos'] = $stmt->fetchAll();
    }
}
...
<head>
    ...
    <style>
        ...
        .sel-ord { padding:8px 12px; border-radius:var(--r); border:1px solid var(--borda); background:var(--branco); font-family:var(--f-corpo); font-size:.83rem; color:var(--escuro); cursor:pointer; }
        .tabela-scroll { overflow-x:auto; }

        .cliente-avatar { width:34px; height:34px; border-radius:50%; background:var(--rosa); color:#fff; font-size:.82rem; font-weight:700; display:flex; align-items:center; justify-content:center; flex-shrink:0; }
        .cliente-rank { display:inline-flex; align-items:center; gap:3px; font-size:.7rem; font-weight:700; color:#f59e0b; margin-top:2px; }
        .cliente-rank svg { width:10px; height:10px; stroke:#f59e0b; fill:#f59e0b; }

        /* Modal perfil */
        .cliente-perfil { display:flex; align-items:center; gap:14px; background:var(--bg); border-radius:10px; padding:14px; margin-bottom:20px; }
        .perfil-avatar { width:48px; height:48px; border-radius:50%; background:var(--rosa); color:#fff; font-size:1.1rem; font-weight:700; display:flex; align-items:center; justify-content:center; flex-shrink:0; }
        .perfil-nome   { font-family:var(--f-titulo); font-size:1.05rem; font-weight:700; }
        .perfil-sub    { font-size:.8rem; color:var(--cinza); margin-top:3px; }
        ...
        .di-lbl  { font-size:.72rem; color:var(--cinza); text-transform:uppercase; letter-spacing:.5px; margin-bottom:4px; }
        .di-val  { font-family:var(--f-titulo); font-size:1.15rem; font-weight:700; }
        .di-val.rosa { color:var(--rosa); }

        /* Últimos pedidos no modal */
        .ped-mini { display:flex; align-items:center; justify-content:space-between; padding:9px 0; border-bottom:1px solid var(--borda); font-size:.82rem; }
        .ped-mini:last-child { border-bottom:none; }
        .ped-mini-id   { font-weight:700; color:var(--rosa); }
        .ped-mini-data { color:var(--cinza); font-size:.76rem; }
        .ped-mini-val  { font-weight:700; }
    </style>
</head>
<body>
        ...
        <div class="topbar">
            <div class="topbar-esq">
                ...
                <div>
                    ...
                    <div class="topbar-breadcrumb"><?= (int)$stats['total'] ?> clientes cadastrados</div>
                </div>
            </div>
            ...
            <div class="toolbar">
                <div class="toolbar-esq">
                    ...
                    $tabs = [
                        ...
                        'ativo'   => 'Ativos ('    . (int)$stats['ativos']  . ')',
                        'inativo' => 'Bloqueados (' . (int)$stats['inativos'] . ')',
                    ];
                    foreach ($tabs as $s => $l):
                    ?>
                ...
                <div class="toolbar-dir">
                    <select class="sel-ord"
                        onchange="window.location='clientes.php?ord='+this.value+'&status=<?= $filtro_status ?><?= $filtro_busca ? '&busca='.urlencode($filtro_busca) : '' ?>'">
                    ...
                    <form method="GET" action="clientes.php" class="busca-wrap">
                        ...
                        <input type="hidden" name="ord"    value="<?= htmlspecialchars($ordenar) ?>">
                        ...
                        <tbody>
                            ...
                            <tr><td colspan="8">
                                <div class="tabela-vazio">
                                    <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="var(--borda)" stroke-width="1.5"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/></svg>
                            ...
                            <tr>
                                ...
                                <td style="font-weight:700"><?= (int)$c['pedidos'] ?></td>
                                <td style="font-family:var(--f-titulo);color:var(--rosa);font-weight:700">
                                    R$ <?= number_format($c['gasto'], 2, ',', '.') ?>
                                </td>
                                ...
                                <td>
                                    <div class="acoes">
                                        <!-- Ver perfil -->
                                        <a href="clientes.php?ver=<?= $c['id'] ?>&status=<?= $filtro_status ?>&ord=<?= $ordenar ?><?= $filtro_busca ? '&busca='.urlencode($filtro_busca) : '' ?>">
                                        ...
                                        <button class="btn-acao danger" title="Excluir"
                                            onclick="confirmarExclusao(<?= $c['id'] ?>, '<?= addslashes(htmlspecialchars($c['nome'])) ?>')">
        ...
        <div class="modal-body">
            ...
            <div class="detalhe-grid">
                <div class="detalhe-item">
                    ...
                    <div class="di-val"><?= (int)$detalhe['pedidos'] ?></div>
                </div>
                <div class="detalhe-item">
                ...
                <div class="detalhe-item">
                    ...
                    <div class="di-val" style="font-size:.95rem"><?= $detalhe['cadastro'] ?? '—' ?></div>
                </div>
                <div class="detalhe-item">
                    <div class="di-lbl">Último pedido</div>
                    <div class="di-val" style="font-size:.95rem"><?= $detalhe['ultimo'] ?? '—' ?></div>
                </div>
            </div>
            ...
            <div class="detalhe-sec">
                ...
                <div class="detalhe-linha">
                    <span>E-mail</span>
                    <span><?= htmlspecialchars($detalhe['email'] ?? '—') ?></span>
                </div>
                <div class="detalhe-linha">
                    <span>Status</span>
                    <span class="badge <?= $detalhe['ativo'] ? 'badge-ativo' : 'badge-inativo' ?>">
            ...
            <!-- Últimos pedidos -->
            <?php if (!empty($detalhe['ultimos_pedidos'])): ?>
            <div class="detalhe-sec">
                <div class="detalhe-sec-titulo">Últimos pedidos</div>
                <?php foreach ($detalhe['ultimos_pedidos'] as $ped):
                    $s = $status_cfg[$ped['status']] ?? $status_cfg['pendente'];
                ?>
                <div class="ped-mini">
                    <div>
                        <span class="ped-mini-id">#<?= $ped['id'] ?></span>
                        <span class="ped-mini-data" style="margin-left:6px"><?= $ped['data'] ?></span>
                    </div>
                    <div style="display:flex;align-items:center;gap:8px">
                        <span class="badge" style="background:<?= $s['bg'] ?>;color:<?= $s['cor'] ?>"><?= $s['label'] ?></span>
                        <span class="ped-mini-val">R$ <?= number_format($ped['total'], 2, ',', '.') ?></span>
                    </div>
                </div>
                <?php endforeach; ?>
            </div>
...
<script>
    ...
    function confirmarExclusao(id, nome) {
        ...
        document.getElementById('confirmTxt').textContent =
            'O cliente "' + nome + '" será removido. Os pedidos existentes serão mantidos sem vínculo.';
